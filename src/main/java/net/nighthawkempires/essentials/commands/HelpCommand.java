                ...
                case 1:
                    for (String pluginName : getPlugins()) {
                        if (args[0].toLowerCase().equals(pluginName.toLowerCase())) {
                            ...
                            if (plugin == null) {
                                sender.sendMessage(getMessages().getChatMessage(GRAY + "I'm sorry, but that is not a valid help topic."));
                                return true;
                            }
                            ...
                            StringBuilder commandBuilder = new StringBuilder();
                            List<String> commands = Lists.newArrayList(description.getCommands().keySet());
                            List<String> availableCommands = getCommands(sender);
                            if (commands != null && !commands.isEmpty()) {
                                for (int i = 0; i < commands.size(); i++) {
                                    if (availableCommands.contains(commands.get(i))) {
                                        commandBuilder.append(GREEN).append(commands.get(i));
                    ...
                    for (String commandName : getCommands(sender)) {
                        if (args[0].toLowerCase().equals(commandName.toLowerCase())) {
                            PluginCommand pluginCommand = Bukkit.getPluginCommand(commandName);
                    ...
                    if (NumberUtils.isDigits(args[0])) {
                        int page = Integer.parseInt(args[0]);
                        if (page > 0 && page <= getTotalPages(sender)) {
                            sendPage(sender, page);
                            return true;
                        } else {
                            sender.sendMessage(getMessages().getChatMessage(GRAY + "I'm sorry, but that is an invalid page number."));
                            return true;
                        }
                    } else {
                        sender.sendMessage(getMessages().getChatMessage(GRAY + "I'm sorry, but that is not a valid help topic."));
